...
def count_kernel(
    ...
    for i in range(num_bins):
        ...
        out_offset = (row_idx * num_blocks_per_row * num_bins) + \
                     (block_idx * num_bins) + \
                     i
...
def scatter_kernel(
    ...
    for i in range(num_bins):
        ...
        offset_idx = (row_idx * num_blocks_per_row * num_bins) + \
                     (block_idx * num_bins) + \
                     i
...
def radix_sort_low_mem(arr, k_bits=4, descending=False): # 建议 k_bits 设小一点，比如 4 或 8
    if arr.ndim == 1:
        arr = arr.unsqueeze(0)
    M, N = arr.shape
    arr_in = arr
    ...
    indices = torch.arange(N, device=arr.device, dtype=torch.int64).broadcast_to(arr.shape).clone()
    idx_in = indices
    ...
    with torch_device_fn.device(arr.device):
        ...
        for p in range(num_passes):
            ...
            count_kernel[grid](
                ...
                M, N, 
                bit_offset, 
                num_bins, BLOCK_N, descending
            )

            total_counts_per_bin = counts.sum(dim=1)
            bin_global_starts = torch.cumsum(total_counts_per_bin, dim=1) - total_counts_per_bin
            block_prefix_sum = torch.cumsum(counts, dim=1) - counts
            global_offsets = bin_global_starts.unsqueeze(1).broadcast_to(block_prefix_sum.shape).clone() + block_prefix_sum
